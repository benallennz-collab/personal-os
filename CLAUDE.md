...
- Never commit `.env` (real Supabase credentials) — only `.env.example` with placeholders. The anon/public key is safe to expose in frontend code (that's what row-level security is for); the `service_role` key must never appear in this codebase.
