...
- `src/components/VoiceAssistant.jsx` — mic button UI. Records speech via the browser's `SpeechRecognition` API, POSTs the transcript + Supabase access token to `/api/voice-command`, displays and speaks (`SpeechSynthesis`) the reply.
...
- Never commit `.env` (real credentials) — only `.env.example` with placeholders. The Supabase anon/public key is safe to expose in frontend code (that's what row-level security is for); the Supabase `service_role` key and `ANTHROPIC_API_KEY` are true secrets and must never appear in client-side code (any `VITE_`-prefixed var), this codebase, or a chat/AI tool — `ANTHROPIC_API_KEY` only ever belongs in `api/voice-command.js`'s server-side `process.env` (set via Vercel's dashboard or a local, gitignored `.env`).
